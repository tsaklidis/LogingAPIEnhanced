...
def detect_dead_sensors():
    ...
    for sensor in active_sensors:
        last_reading = SensorReading.objects.filter(sensor=sensor).order_by("-recorded_at").first()
...
def cleanup_old_readings(days=90):
    ...
    deleted_count, _ = SensorReading.objects.filter(recorded_at__lt=cutoff).delete()
